...
export default function Testimonials() {
  ...
  const formatGrantType = (type?: string | null) => {
    if (!type) return "Education Grant";

    return (
      grantTypeLabels[type] ||
      type
        .split("_")
      ...
      <div className="bg-primary text-primary-foreground py-24 px-4 text-center">
        <div className="container mx-auto max-w-4xl">
          ...
          <p className="text-xl text-primary-foreground/80 max-w-2xl mx-auto font-light leading-relaxed">
            Real stories from college students across America who received
            grants from the Hope Foundation — and what it meant for their
            academic futures.
          </p>
        </div>
      </div>
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {Array.from({ length: 6 }).map((_, i) => (
              <div
                key={i}
                className="bg-card p-8 rounded-2xl border border-border h-72 animate-pulse shadow-sm"
              />
            ))}
          </div>
            ...
            {testimonials.map((t) => (
              ...
              >
                ...
                <blockquote className="text-foreground/80 text-base leading-relaxed mb-8 flex-grow relative">
                  <span className="text-4xl text-primary/10 absolute -top-4 -left-2 font-serif">
                    "
                  </span>

                  <span className="relative z-10">
                    {t.message || "No testimonial provided."}
                  </span>
                </blockquote>
                ...
                <div className="flex items-center gap-4 mt-auto pt-6 border-t border-border/50">
                  ...
                  <div>
                    <div className="font-bold font-serif text-foreground tracking-tight">
                      {t.name || "Anonymous"}
                    </div>

                    <div className="text-sm text-muted-foreground font-medium">
                      {t.location || "United States"}
                    </div>
                    <div className="text-xs text-primary/70 font-medium mt-0.5">
                      {formatGrantType(t.grantType)}
                    </div>
                  </div>
                </div>
              </motion.div>
          ...
          <div className="text-center py-20">
            ...
            <p className="text-muted-foreground mb-8">
              Check back soon — more student stories are on the way.
            </p>
          </div>
        )}
        ...
        {!isLoading && (
          <div className="mt-20 text-center bg-muted/40 p-12 rounded-3xl border border-border max-w-4xl mx-auto">
            ...
            <p className="text-muted-foreground mb-8 text-lg max-w-xl mx-auto">
              Financial barriers should never stop a great student. Apply today
              and let us help you reach the finish line.
            </p>
